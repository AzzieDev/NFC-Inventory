...
// Do not auto-fill item description from query parameters; only use database existing record if already bound
$nameVal = htmlspecialchars((string) ($existing['friendly_name'] ?? ''), ENT_QUOTES, 'UTF-8');
...
<body class="text-slate-100 font-sans p-4 md:p-8 flex items-center justify-center min-h-screen">
    <main class="max-w-xl w-full mx-auto glass rounded-3xl p-6 md:p-8 space-y-6 border border-slate-700">
        ...
        <form id="bindForm" action="<?= htmlspecialchars($formAction, ENT_QUOTES) ?>" method="POST" class="space-y-5">
            ...
            <!-- Friendly Display Name / Item Description (Optional, Ordered Before Slug) -->
            <div>
                <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Item Title / Description (Optional)</label>
            ...
            <!-- Friendly Slug with Auto-Gen (Optional) -->
            <div>
                <div class="flex justify-between items-center mb-1">
                    <label class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Custom Friendly Slug / URL (Optional)</label>
                ...
                <input type="text" id="slugInput" name="slug" value="<?= $slugVal ?>" placeholder="e.g. storage-bin-01 or favorite-item" class="w-full bg-slate-900 border border-slate-700 rounded-xl px-4 py-3 font-mono text-sm text-slate-200 focus:border-indigo-500 focus:outline-none transition">
            ...
            <div>
                ...
                <input type="text" name="target_url" value="<?= $urlVal ?>" placeholder="https://, tel:, mailto: (Optional)" class="w-full bg-slate-950 border border-cyan-500/50 rounded-xl px-4 py-3 text-sm text-cyan-300 focus:border-cyan-400 focus:outline-none transition">
    ...
    <!-- Optional Fields Confirmation Modal -->
    <div id="optionalModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 hidden flex items-center justify-center p-4 opacity-0 transition-opacity duration-200">
        <div class="glass max-w-lg w-full rounded-3xl p-6 md:p-8 border border-indigo-500/40 shadow-2xl shadow-indigo-500/10 scale-95 transition-transform duration-200" id="optionalModalBox">
            <div class="flex items-center gap-4 border-b border-slate-800 pb-4 mb-5">
                <div class="w-12 h-12 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-400 flex items-center justify-center text-xl shrink-0">
                    <i class="fa-solid fa-circle-question"></i>
                </div>
                <div>
                    <span class="text-xs font-bold text-indigo-400 uppercase tracking-wider block">Optional Fields Unset</span>
                    <h3 class="text-xl font-bold text-white">Confirm Empty Parameters</h3>
                </div>
            </div>
            <p id="optionalModalText" class="text-slate-300 text-sm leading-relaxed mb-6"></p>
            <div class="flex flex-col sm:flex-row items-center justify-end gap-3 pt-2 border-t border-slate-800">
                <button type="button" onclick="closeOptionalModal(false)" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 font-semibold text-sm transition">
                    <i class="fa-solid fa-xmark mr-1.5"></i> Cancel &amp; Edit
                </button>
                <button type="button" onclick="closeOptionalModal(true)" class="w-full sm:w-auto px-5 py-2.5 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-500 hover:brightness-110 active:scale-95 text-white font-bold text-sm shadow-lg shadow-indigo-500/20 transition">
                    <i class="fa-solid fa-check mr-1.5"></i> Save Anyway
                </button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let optionalResolve = null;
        function showOptionalDialog(missingType) {
            return new Promise((resolve) => {
                optionalResolve = resolve;
                const modal = document.getElementById('optionalModal');
                const box = document.getElementById('optionalModalBox');
                const textEl = document.getElementById('optionalModalText');
                
                if (missingType === 'both') {
                    textEl.textContent = "You have left both Item Description and Custom Friendly Slug empty. If you proceed, the chip will be saved using only its raw hardware serial UID without a display title or shortcut slug.";
                } else if (missingType === 'description') {
                    textEl.textContent = "You have left Item Description empty. If you proceed, this chip will be stored in inventory without a descriptive display label.";
                } else {
                    textEl.textContent = "You have left Custom Friendly Slug empty. If you proceed, this chip will only be routable via its raw hardware serial UID.";
                }
        ...
        function closeOptionalModal(confirmed) {
            const modal = document.getElementById('optionalModal');
            const box = document.getElementById('optionalModalBox');
            ...
            setTimeout(() => {
                ...
                if (optionalResolve) optionalResolve(confirmed);
            }, 200);
        }
        ...
        let isConfirmedOptional = false;
        
        document.getElementById('bindForm').addEventListener('submit', async function(e) {
            if (isConfirmedDuplicate && isConfirmedOptional) return;
            
            e.preventDefault();
            const btn = document.getElementById('submitBtn');
            const descVal = nameInput.value.trim();
            let slugVal = slugInput.value.trim();
            
            if (slugVal === '' && descVal !== '') {
                slugVal = descVal.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '');
                slugInput.value = slugVal;
            }
            
            // 1. Optional fields verification
            if (!isConfirmedOptional && (descVal === '' || slugVal === '')) {
                let missingType = 'both';
                if (descVal === '' && slugVal !== '') missingType = 'description';
                else if (descVal !== '' && slugVal === '') missingType = 'slug';
                
                const confirmedOpt = await showOptionalDialog(missingType);
                if (!confirmedOpt) return;
                isConfirmedOptional = true;
            } else {
                isConfirmedOptional = true;
            }
            
            // 2. Duplicate target verification
            if (!isConfirmedDuplicate) {
                ...
                try {
                    ...
                    if (data.status === 'success' && data.has_duplicates) {
                        ...
                        const confirmedDup = await showDuplicateDialog(data.duplicates);
